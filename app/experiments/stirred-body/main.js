...
//
// Undulation depth follows how much the orb is moving *at all* -- rotation and
// travel through space, both direction-agnostic. Held still it settles almost
// smooth; moved, it churns. Rotation still drags directional ripples on top.
//
// Rotation and translation are weighted separately because they are not
// comparable quantities: deg/s and m/s^2 have no honest conversion, and any
// single combined number would need an arbitrary radius to relate them.
...
async function main() {
  ...
  const hud = mountTopbar({ title: 'stirred body', backend: stage.backend });
  // Forked from the viscous body: starts from wherever that one was tuned to,
  // then keeps its own settings once anything here is touched.
  const panel = createPanel({
    title: 'Stirred body',
    storageKey: 'stirred-body',
    inheritFrom: 'viscous-body',
  });
  ...
  stage.onUpdate((dt) => {
    ...
    if (m.valid) {
      ...
      // How much the orb is moving, however it is moving. Kept as two separate
      // gates rather than one fused number: an object can be carried across a
